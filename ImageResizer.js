...
{
	//this.SEAM_URL = "http://localhost/seam/image";
	this.SEAM_URL = "seamJSON.js"
	//console.log("Initializing " + canvasDiv + " with image " + imgUrl);
	this.imgUrl = imgUrl;
...
ImageResizer.prototype.drawImage = function(imageUrl)
{
	...
	img.onload = function()
	{
		...
		imgResizer.imageData = imgResizer.canvasContext.getImageData(0,0,img.width,img.height);
		imgResizer.originalImageData = imgResizer.canvasContext.getImageData(0,0,img.width,img.height);
		imgResizer.getSeams(imageUrl);
	};
};

/**
 * Gets the seams of the current image by analysing the image 
 * on the server. This uses AJAX for doing this
 */
ImageResizer.prototype.getSeams = function(imageUrl)
{
	...
	var imgResizer = this;
	xmlHttp.onreadystatechange = function(){imgResizer.onGetSeamHandler.call(imgResizer,xmlHttp);}
...
ImageResizer.prototype.resizeReduce= function(widthDiff, heightDiff)
{
	...
	this.canvasContext.clearRect(this.originalWidth-widthDiff, 0, this.originalWidth, this.originalHeight);
	this.originalWidth = this.originalWidth - widthDiff;
	//console.log("Resize complete from  (" + this.canvasDiv.width + ", " + this.canvasDiv.height + ") to (" + x1 + ", " + y + ")");
};
...
ImageResizer.prototype.showSeam = function(n)
{
	...
	{
		var color = {'red' : 0, 'blue' : 100, 'green': 100, 'alpha' : 1};
 ...
 * @param {int} x
 * @param {int} y
 * @param {rgba  {'red' : 'int', 'blue':'int'; 'green' : 'int', 'alpha': ''}} rgba
 */
